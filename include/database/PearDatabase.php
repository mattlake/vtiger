...
class PearDatabase implements DatabaseContract
{
    ...
    public $log = null;
    public int $lastmysqlrow = -1;
    public bool $enableSQLlog = false;
    public bool $continueInstallOnError = true;
    ...
    public function __construct($dbtype = '', $host = '', $dbname = '', $username = '', $passwd = '')
    {
        ...
        $this->is_db_default_utf8_charset = PerformancePrefs::getBoolean('DB_DEFAULT_CHARSET_UTF8');
    ...
    public function connect($dieOnError = false)
    {
        ...
        if ($result) {
            ...
            if ($this->is_db_default_utf8_charset) {
    ...
    public function executeSetNamesUTF8SQL($force = false)
    {
        ...
        if ($DEFAULTCHARSET == 'UTF-8' && ($force || ! $this->is_db_default_utf8_charset)) {
    ...
    public function checkError($msg = '', $dieOnError = false)
    {
        if ($this->dieOnError || $dieOnError) {
            ...
            echo '</pre>';
            $this->println(
                "ADODB error " . $msg . "->[" . $this->database->ErrorNo() . "]" . $this->database->ErrorMsg()
            );
    ...
    public function updateBlobFile($tablename, $colname, $id, $filename)
    {
        ...
        $this->println(
            "updateBlobFile t=" . $tablename . " c=" . $colname . " id=" . $id . " f=" . $filename . " status=" . $result
        );
        return $result;
    }
    ...
    public function change_key_case($arr): array
    {
        ...
        if (is_array($data)) {
            switch ($data['type']) {
                ...
                case 'timestamp':
                    ...
                    break;
                default:
                    throw new Exception("unhandled type: " . serialize($data['type']));
            }
        } else {
    ...
    public function sql_expr_datalist($a): string
    {
        if (! is_array($a)) {
    ...
    public function sql_expr_datalist_from_records($a, $field): string
    {
        if (! is_array($a)) {
            throw new Exception("not an array");
        }
    ...
    public function sql_concat($list): string
    {
        switch ($this->dbType) {
            case 'mysql':
    ...
    public function query_result_rowdata(&$result, $row = 0): array
    {
        if (! is_object($result)) {
            throw new Exception("result is not an object");
        }
        ...
        foreach ($rowData as $col => $columnData) {
            if ($col != 'fieldlabel') {
                $rowData[$col] = to_html($columnData);
            }
        }
        return $rowData;
    }
     ...
     * @param $row integer The row number to fetch. It's default value is 0
     *
     */
    ...
    public function requireSingleResult($sql, $dieOnError = false, $msg = '', $encode = true): string
    {
    ...
    public function requirePsSingleResult($sql, $params, $dieOnError = false, $msg = '', $encode = true): string
    {
    ...
    public function convert2Sql($ps, $vals)
    {
        ...
        $sql = preg_replace_callback(
            "/('[^']*')|(\"[^\"]*\")|([?])/",
            array(new PreparedQMark2SqlValue($vals), "call"),
            $ps
        );
        return $sql;
    }
    ...
    {
        $result_data = mysqli_real_escape_string(
            $this->database->_connectionID,
            $str
        );
    ...
    public function fetchByAssoc(&$result, $rowNum = -1, $encode = true): ?array
    {
        if ($result->EOF) {
    ...
    public function fetch_row(&$result, $encode = true): ?array
    {
        return $this->getNextRow($result);
    }

    public function getNextRow(&$result, $encode = true): ?array
    {
        global $log;
        $log->info('getNextRow');
        if (isset($result)) {
    ...
    public function alterTable($tablename, $flds, $oper)
    {
        ...
        $this->println(
            "ADODB alterTableTable table=" . $tablename . " flds=" . $flds . " oper=" . $oper . " status=" . $result
        );
        return $result;
    }
    ...
    public function formatString($tablename, $fldname, $str): string
    {
        $this->checkConnection();
        $adoflds = $this->database->MetaColumns($tablename);
        ...
        foreach ($adoflds as $fld) {
            if (strcasecmp($fld->name, $fldname) == 0) {
                ...
                if (strcmp($fldtype, 'CHAR') == 0 || strcmp($fldtype, 'VARCHAR') == 0 || strcmp(
                        $fldtype,
                        'VARCHAR2'
                    ) == 0 || strcmp($fldtype, 'LONGTEXT') == 0 || strcmp($fldtype, 'TEXT') == 0) {
